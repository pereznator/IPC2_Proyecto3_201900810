...
class Estadistica(Resource):
    ...
    def __init__(self):
        ...
        self.est_post.add_argument("eventos", action='append',help="No se enviaron los eventos", required=True)

    def get(self):
        base = path.dirname(__file__)
        ruta = path.abspath(path.join(base, '..', '..', 'estadisticas.xml'))
        estadisticas_texto = ''
        with open(ruta, 'r', encoding='utf8') as doc:
            estadisticas_texto = doc.read()
        return {"mensaje": "exito", "data": estadisticas_texto}
            ...
            for estadistica in raiz.findall('ESTADISTICA'):
                ...
                if fecha in nf:
                    ...
                    for usuario in usuarios.findall('USUARIO'):
                        if usuario.find('EMAIL').text in evento['reportadoPor']:
    ...
    def put(self):
        ...
        ruta = path.abspath(path.join(base, '..', '..', 'estadisticas.xml'))
        data = ''
        with open(ruta, 'w') as doc:
            data = '<ESTADISTICAS>\n</ESTADISTICAS>'
            doc.write(data)
        return {"mensaje": "Exito", "data": data}
